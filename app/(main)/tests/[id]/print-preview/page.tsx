...
export default function PrintPreviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const groupPoints = groups.length ? distribute(totalPoints, groups.length) : [];
  ...
  async function handleDownload(format: "pdf" | "word") {
    ...
    setMessage(null);
    try {
      const request = { problem_ids: selectedIds, duration_minutes: duration, total_points: totalPoints, title };
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[320px_minmax(0,1fr)]">
        <aside className="sticky top-4 border border-slate-200 bg-white p-4">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            ...
            <label className="text-sm text-slate-600">満点<Input className="mt-1" type="number" min={1} max={1000} value={totalPoints} onChange={(event) => setTotalPoints(Number(event.target.value))} /></label>
          </div>
        ...
        {selectedProblems.length === 0 ? <EmptyBlock label="問題を選ぶと、ここに用紙が表示されます" /> : (
          <section className="mx-auto w-full max-w-[794px] bg-white px-[68px] py-[52px] text-black shadow-sm ring-1 ring-slate-200" style={{ fontFamily: '"Yu Gothic", "BIZ UDPGothic", sans-serif' }}>
            ...
            <div className="mt-5 grid grid-cols-[1fr_auto_auto] gap-6 text-sm"><span>実施日：____年__月__日</span><span>制限時間：{duration}分</span><span>満点：{totalPoints}点</span></div>
            ...
            {groups.map((group, groupIndex) => (
              <div key={groupIndex} className="mb-8 break-inside-avoid">
                ...
                {group.map((problem, problemIndex) => <div key={problem.id} className="mb-7"><p className="whitespace-pre-wrap text-[15px] leading-7">({problemIndex + 1})　{problem.body}</p><div className="h-16" /></div>)}
              </div>
            ))}
