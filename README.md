...
Geminiが返した問題は、保存・表示の前に「生成が完了しているか」を確認します。判定は `providers.py` の `UNFINISHED_PATTERNS` で、**未完成であることが明らかな痕跡だけ**を弾きます。

| 弾く | 例 |
|---|---|
| プレースホルダ | `TODO` / `TBD` / `FIXME` / `PLACEHOLDER` |
| 入力欄の雛形 | 「ここに問題を入力」「ここに解答を記入」 |
| 指示文がそのまま出力 | 「解説を生成してください」 |
| 未置換テンプレート変数 | `{{problem_body}}` / `[[...]]` |
| 思考途中の独り言 | 「おっと、正しくは…」「すみません」 |
| 空・欠落・壊れたJSON | 問題文・正答・解説が空、ヒントが3段ない、必須フィールド欠落 |

「確認してください」「訂正すると」「もう一度計算すると」「計算ミスに注意」のような**正常な数学の言い回しは弾きません**。以前は単純な部分文字列一致で `確認` `訂正` `もう一度` 等を検出しており、正しい解説まで弾いて「数学校閲後も問題文・正答・解説の検証条件を満たしませんでした。」で生成が止まる原因になっていました。

未完成の問題が見つかった場合、**バッチ全体を捨てずにその問題だけを作り直します**（`MAX_PROBLEM_REGENERATIONS` 回まで）。単元・形式・難易度は変えません。上限まで試しても直らなければ従来どおりエラーになります。ログには `regenerating unfinished problem #3 (...) attempt 1/2` が残ります。
